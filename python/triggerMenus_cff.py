...
class TriggersByDataset:
    ...
    def get_dataset(self,dataset):
        if(dataset=='MC'):
            return { 'pass_triggers' : list(self.all_triggers) , 'veto_triggers':[] , 'ignore_triggers' : [] }
        else:
            if 'ignore_triggers' not in self.triggerMenu[dataset]:
                self.triggerMenu[dataset]['ignore_triggers']=[trigger for trigger in self.all_triggers if (trigger not in self.triggerMenu[dataset]['pass_triggers'] and trigger not in self.triggerMenu[dataset]['veto_triggers'])]
            try:
